...
titles <- c("Mainroad", "Guestroom", "Basement", "Hot Water Heating", "Air Conditioning", "Preferred Area")
...
for (i in seq_along(yes_no_vars)) {
  ...
  p <- ggplot(data, aes(x = price, color = .data[[var]], fill = .data[[var]])) +
    ...
    geom_vline(data = group_means, aes(xintercept = grp.mean, color = .data[[var]]), linetype = "dashed") +
...
p <- ggplot(data, aes(x = price, color = furnishingstatus, fill = furnishingstatus)) +
    geom_histogram(bins = 20, alpha = 0.6) +
    geom_vline(data = furnishing_means, aes(xintercept = grp.mean, color = furnishingstatus), linetype = "dashed") +
    scale_fill_brewer(palette = "Paired") +  # Using 'Paired' to visually distinguish categories clearly
...
model_train <- lm(price ~ area + bathrooms + airconditioning + stories, data = train_data)
